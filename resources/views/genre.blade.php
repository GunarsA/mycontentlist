...
<div class="container mx-auto">
    ...
    <a href="{{ action([App\Http\Controllers\GenreController::class, 'create']) }}" class="inline-block px-4 py-2 mb-4 bg-blue-500 text-white rounded hover:bg-blue-600">New Genre</a>
    <ul class="space-y-2">
        ...
        <li class="flex items-center justify-between bg-white rounded-lg p-4 shadow-md">
            ...
            <form method="POST" action="{{ route('genre.destroy', $genre->id) }}" class="inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" value="delete" class="text-red-500 hover:text-red-700">Delete</button>
            </form>
        </li>
        @endforeach
    </ul>
